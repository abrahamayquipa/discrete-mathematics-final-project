...
Ciudad matrizMapa3[25][25] = {
	...
	/*Bogota			*/	 { Ciudad("Bogota", "Armenia", 1),          Ciudad("Bogota", "Bucaramanga", 1), 			Ciudad("Bogota", "Barrancabermeja", 1), 			Ciudad("Bogota", "Barranquilla", 1), 			Ciudad("Bogota", "Bogota", 0), 				Ciudad("Bogota", "Cucuta", 1), 					Ciudad("Bogota", "Cartagena", 1), 				Ciudad("Bogota", "Cali", 1), 			Ciudad("Bogota", "Florencia", 1), 			Ciudad("Bogota", "Ibague", 1), 			Ciudad("Bogota", "Leticia", 1), 			Ciudad("Bogota", "Monteria", 1), 				Ciudad("Bogota", "Manizales", 1), 				Ciudad("Bogota", "Medellin", 1), 				Ciudad("Bogota", "Neiva", 1), 				Ciudad("Bogota", "Pasto", 1), 					Ciudad("Bogota", "Popayan", 1), 			Ciudad("Bogota", "Pereira", 1), 				Ciudad("Bogota", "Riohacha", 1), 				Ciudad("Bogota", "San Andres", 1), 					Ciudad("Bogota", "Santa Marta", 1), 			Ciudad("Bogota", "Tumaco", 0), 				Ciudad("Bogota", "Valledupar", 1), 					Ciudad("Bogota", "Villavicencio", 1), 					Ciudad("Bogota", "Yopal", 1),},
...
// Matrices con valores númerico
int matriz3[25][25];
int matrizCuadrada3Numerica[25][25];
int matrizCubica3Numerica[25][25];
int matrizUnaEscalaEnMatriz3[25][25];
int matrizDosEscalasEnMatriz3[25][25];

string funcionObtenerNombreMatriz3(int n) {
	string nombre;
	switch (n) {
...
void funcionRestarMatriz3(int e) {
	for (int i = 0; i < 25; i++) {
		for (int j = 0; j < 25; j++) {
			matrizUnaEscalaEnMatriz3[i][j] = matrizCuadrada3Numerica[i][j] - matriz3[i][j];
			if (i == j) {
				matrizUnaEscalaEnMatriz3[i][j] = 0;
			}
			if (matrizUnaEscalaEnMatriz3[i][j] <= 0) {
				matrizUnaEscalaEnMatriz3[i][j] = 0;
			}
			if (e == 2) {
				matrizDosEscalasEnMatriz3[i][j] = matrizCubica3Numerica[i][j] - matrizUnaEscalaEnMatriz3[i][j];
				matrizDosEscalasEnMatriz3[i][j] = matrizDosEscalasEnMatriz3[i][j] - matrizCuadrada3Numerica[i][j];
				matrizDosEscalasEnMatriz3[i][j] = matrizDosEscalasEnMatriz3[i][j] - matriz3[i][j];
				if (i == j) {
					matrizDosEscalasEnMatriz3[i][j] = 0;
				}
				if (matrizDosEscalasEnMatriz3[i][j] <= 0) {
					matrizDosEscalasEnMatriz3[i][j] = 0;
				}
			}
		}
...
void funcionMatrizCuadrado3() {
	for (int i = 0; i < 25; i++) {
		for (int j = 0; j < 25; j++) {
			for (int k = 0; k < 25; k++) {
				matrizCuadrada3Numerica[i][j] += matriz3[i][k] * matriz3[k][j];
				if (matrizCuadrada3Numerica[i][j] >= 1) {
					matrizCuadrada3Numerica[i][j] = 1;
				}
			}
		}
	}
...
void funcionMatrizCubo3() {
	for (int i = 0; i < 25; i++) {
		for (int j = 0; j < 25; j++) {
			for (int k = 0; k < 25; k++) {
				matrizCubica3Numerica[i][j] += matrizCuadrada3Numerica[i][k] * matriz3[k][j];
				if (matrizCubica3Numerica[i][j] >= 1) {
					matrizCubica3Numerica[i][j] = 1;
				}
			}
		}
	}
...
void funcionUnaEscalaMatriz3() {
	funcionRellenarMatriz3();
	funcionMatrizCuadrado3();
	funcionRestarMatriz3(1);
	for (int i = 0; i < 25; i++) {
		for (int j = 0; j < 25; j++) {
			int v = matrizUnaEscalaEnMatriz3[i][j];
			string o = funcionObtenerNombreMatriz3(i);
			string d = funcionObtenerNombreMatriz3(j);
...
void funcionDosEscalasMatriz3() {
	funcionRellenarMatriz3();
	funcionMatrizCuadrado3();
	funcionMatrizCubo3();
	funcionRestarMatriz3(2);
	for (int i = 0; i < 25; i++) {
		for (int j = 0; j < 25; j++) {
			int v = matrizDosEscalasEnMatriz3[i][j];
			string o = funcionObtenerNombreMatriz3(i);
			string d = funcionObtenerNombreMatriz3(j);
...
string funcionObtenerPuntoPasoMatriz3(string origen, string destino) {
	string result;
	int auxI, auxJ, p1, p2;
	bool pp1 = true;
	bool pp2 = false;
	for (int i = 0; i < 25; i++) {
		for (int j = 0; j < 25; j++) {
			if (origen == matrizMapa3[i][j].getOrigen()) {
				auxI = i;
			}
			if (destino == matrizMapa3[i][j].getOrigen()) {
				auxJ = i;
			}
		}
	}
	/**************************  Dos escalas ****************************/
	for (int j = 0; j < 25; j++) {
		if (matrizMapa3[auxI][j].getValor() == 1) {
			p1 = j;
		}
	}

	for (int j = 0; j < 25; j++) {
		if (matrizMapa3[auxJ][j].getValor() == 1) {
			p2 = j;
		}
	}
	/**************************  Una escala  ****************************/
	for (int i = 0; i < 25; i++) {
		...
		if ((matrizMapa3[auxI][i].getValor() == matrizMapa3[auxJ][i].getValor()) && (matrizMapa3[auxJ][i].getValor() == 1 && matrizMapa3[auxI][i].getValor() == 1)) {
			result = matrizMapa3[i][i].getOrigen();
			return result;
		}else { pp2 = true; pp1=false; }
		// Punto de paso de dos escalas
		if (matrizMapa3[p1][p2].getValor() && pp2) {
			result = matrizMapa3[p1][p1].getOrigen() + " -> " + matrizMapa3[p2][p2].getOrigen();
			return result;
		}
	}
}